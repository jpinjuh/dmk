...
// Actions
import { activateData } from "Modules/units/Archdioceses";
...
const ActivateModal = ({ onActivate, closeActivate }) => {
  ...
  const activateItem = (e) => {
    ...
    dispatch(activateData(`archdiocese/activate`, {id: oneItem.id}))
    closeActivate();
  }
            ...
            <Box display="flex" flexDirection="column" p={2}>
              <Box mb={3}>
                <Title 
                  variant="h6" 
                  align={'left'} 
                  title={'Jeste li sigurni da želite aktivirati odabranu biskupiju?'}
                />
              </Box>
